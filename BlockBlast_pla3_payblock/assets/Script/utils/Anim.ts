...
export default class Anim {
     ...
     */
    public ShowFlyAni(prefab: cc.Prefab, parent, len: number, endPoint: cc.Vec2,comFun: Function = null, thisObj: any = null,originPos?:cc.Vec2) {
        this.count = 0;
        this._staLen = len;
        this._fun = comFun;
        this._thisObj = thisObj;
        let halfX: number = 0;
        if(cc.winSize.width>cc.winSize.height)halfX = cc.winSize.width / 5
        let halfY: number = 0;
        if(originPos!=null){
            halfX = originPos.x
            halfY = originPos.y
     ...
     */
    public sequenceAnim(node:cc.Node,ActionArr:any,callback?:Function){
        let func = null
        let seq = null
        if(callback!=null){
            func = cc.callFunc(callback,this)
            seq = cc.sequence([...ActionArr,func])
            node.runAction(seq)
        }else{
            seq = cc.sequence([...ActionArr])
            node.runAction(seq)
        }
        
    }
    private shakeEffect(node:cc.Node,duration) {
        node.runAction(
            cc.repeatForever(
                cc.sequence(
