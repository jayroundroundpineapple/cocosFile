 ...
 */
   ...
   * @returns {number}
   */
  public static showUI(node: cc.Node, audio: cc.AudioClip, delay: number,canPlayMusic:boolean,callback?: Function) {
    setTimeout(() => {
      canPlayMusic && cc.audioEngine.play(audio, false, 1)
      node.active = true
      cc.tween(node)
        .to(0.3, { scale: 1.1 }, { easing: 'sineOutIn' })
        .to(0.2, { scale: 1 }, { easing: 'sineOutIn' })
        .call(() => {
