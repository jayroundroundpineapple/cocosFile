...
export class LanguageCount extends cc.Component {
    ...
    private ChangeLanguage(): void {
        ...
        if(this.autoPrefix){
            this.lable.string = `${this.prefix}${unit}${mgr.formatUnit(this.languageNum)}.00${this.endFix}`;
        }else{
            this.lable.string = `${this.prefix}${mgr.formatUnit(this.languageNum)}.00${this.endFix}`;
        }
        if(this.autoEndfix){
            this.lable.string = `${this.prefix}${mgr.formatUnit(this.languageNum)}.00${this.endFix}${unit}`;
        }
    }
}
