...
export default class GameUI extends cc.Component {
    ...
    protected start(): void {
        ...
        this.honMoneyLabel.string = `${this.moneyChange.prefix}${this.amount}.00`
    ...
    private initBlock() {
        ...
        for (let i = 0; i < totalNum; i++) {
            ...
            item.initItem(i, this._data[i].spriteIndex, pos, this._data[i].isEmpty, () => {
    ...
    private fallDown() {
        ...
        // setTimeout(() => {
        //     for(let i = 0;i < this.NodeArr.length;i++){
        //         this.NodeArr[i].getComponent(cc.Sprite).spriteFrame = null
        //         this.NodeArr[i].children[0].active = true
        //     }
        //     for(let i = 0;i < this.RowColmun;i++){
        //         for(let j =0;j<this.RowColmun;j++){
        ...
        //         Utils.showUI(this.BoxUI,RESSpriteFrame.instance.comeOutClip,0.5,this.canPlayMusic,resultNodeScale,resultNodeScale1,()=>{
        //             this.coinNode.active = true
        //             this.BoxUI.active = false
        //             Utils.showUI(this.resultNode,RESSpriteFrame.instance.comeOutClip,0.2,this.canPlayMusic,resultNodeScale,resultNodeScale1,()=>{
        //                 console.log(1);
        //                 let index = 0
    ...
    private DoFallAnim(row: number, colmun: number, startIndex: number, gap: number) {
        for (let i = 0; i < startIndex; i++) {
            ...
            cc.tween(item.node)
                .to(.25, { y: item.y - 65 * gap }, { easing: "quadIn" })
                .call(() => {
                    console.log('jayza',i,colmun,this.allItemArr[i][colmun]);
    ...
    private createSecond() {
        ...
        for (let i = 0; i < this.RowColmun; i++) {
            for (let j = 0; j < this.RowColmun; j++) {
                if (this.allItemArr[i][j].isEmpty) {
                    ...
                    item.setPostionY(item.y + 65 * 6)
                    item.setSprite()
                    item.node.opacity = 255
                    item.node.parent = this.BoxBg
                    cc.tween(item.node)
                        .to(.45, { y: item.originPosY + 130 }, { easing: 'quadIn' }).
                        call(() => {
                            this.allItemArr[i][j].isEmpty = false
                        })
                        .start()
        ...
        // for (let i = 0; i < this.NodeArr.length; i++) {
        //     this.NodeArr[i].active = true
        //     cc.tween(this.NodeArr[i])
        //         .to(.45, { y: this.NodeArr[i].y - 65 * 5 }, { easing: 'quadIn' })
        //         .start()
        // }
    }
    ...
    private secondClickFunc(node: cc.Node) {
        if(this.secondFlag){
